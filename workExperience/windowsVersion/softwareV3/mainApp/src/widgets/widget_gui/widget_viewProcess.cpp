...
void widget_viewProcess::_showParam(process_type type) {
	...
	case process_type::Colourisation:
		...
		lineEditMask->setText(parent->getMask());
		...
		//frameRTKSetting->hide();
		//frameColourSetting->show();
		//frameGcpSetting->hide();

		//tabSetting->setCurrentIndex(1);
		break;
...
void widget_viewProcess::setSlamParam(QStringList slam) {
	...
	if (slam.at(25) == "true")
		labelInterior->setText("true");
	else
		labelInterior->setText("false");
