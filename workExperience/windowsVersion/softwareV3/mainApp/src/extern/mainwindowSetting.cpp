...
void mainwindowSettingDialog::init() {
    ...
    connect(pushButtonCmd, SIGNAL(clicked()), this, SLOT(btnCmd_triggered()), Qt::UniqueConnection);
    connect(pushButtonUpdateTime, SIGNAL(clicked()), this, SLOT(btnUpdate_triggered()), Qt::UniqueConnection);
...
void mainwindowSettingDialog::rescaleIntensity(QString inputFile) {
    if (!rescaleIn) {
        ...
        QFileInfo f("..\\Lib\\company\\las_rescale.exe");
        QString rescaleExe = f.absoluteFilePath().replace('/', '\\');
        widget_newProcess* forQuote = new widget_newProcess(nullptr);
        rescaleExe = forQuote->addQuote(rescaleExe.toStdString());

        QString command = QString(rescaleExe + " %1 %2 -p 1\n")
            .arg(forQuote->addQuote(inputFile.toStdString()))
            .arg(forQuote->addQuote(outputFile.toStdString()));
...
void mainwindowSettingDialog::bagReindex(QString inputFile) {
    if (!bagReindex_) {
        ...
        widget_newProcess* forQuote = new widget_newProcess(nullptr);
        ros_setup = forQuote->addQuote(ros_setup.toStdString());

        QString command = QString("rosbag info %1").arg(forQuote->addQuote(inputFile.toStdString()));
        bagReindex_->write(QString(ros_setup + "\r\n").toUtf8());
        bagReindex_->write(QString(command + "\r\n").toUtf8());

        std::cout << "info first" << std::endl;
        labelBagRe->setText(tr("Checking the bag.."));
...
void mainwindowSettingDialog::bagReindex2() {
    if (!bagReindex2_) {
        ...
        QString command = QString("rosbag reindex %1").arg(forQuote->addQuote(bagReindexFile.toStdString()));
        //bagReindex2_->write(QString(ros_setup + "\r\n").toUtf8());
        ...
        if (fi.exists()) {
            ...
            bagReindex2_->start(QString(ros_setup + " && " + command + "\n"));
...
void mainwindowSettingDialog::btnCmd_triggered() {

    if (parent_->checkCmd()) {
        emit(_cmd());
    }
    else {
        QMessageBox::information(0, QString(tr("Warning")), QString(tr("It is already opened.")));
    }

}

void mainwindowSettingDialog::btnUpdate_triggered() {

    if (lineEditUpdateTime->text().size() == 20) {
        int res = parent_->login_page_one.login_checker_one.checkAuthentity(lineEditUpdateTime->text(), false);
        if (res == 6) {
            QMessageBox::information(NULL, tr("Information"), tr("Set Succeed!") + ("\r\r"));
        }
        else {
            QMessageBox::information(NULL, tr("Information"), tr("Set Failed.")+("\r\r"));
        }
    }
}