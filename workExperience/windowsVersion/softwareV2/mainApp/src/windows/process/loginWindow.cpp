#include "../../include/process/loginWindow.h"
    ...
    : QDialog(parent)
{
    ...
    connect(ui.pushButton_2, SIGNAL(clicked()), this, SLOT(_quit_clicked()), Qt::UniqueConnection); // to show if the license entered is valid
    connect(ui.pushButton_en, SIGNAL(clicked()), this, SLOT(_btnen_clicked()), Qt::UniqueConnection); // to show if the license entered is valid
    connect(ui.pushButton_zh, SIGNAL(clicked()), this, SLOT(_btnzh_clicked()), Qt::UniqueConnection); // to show if the license entered is valid
...
void windowLogin::paintEvent(QPaintEvent* event)
{
    ...
    QPixmap pix;
    pix.load(":/process/process/logo.png");
...
void windowLogin::slot_updataUI()
{
    ...
    if (ui.lineEdit_2->text() == "")
    {
        ...
        datasetbtn->setStyleSheet("QPushButton{border-image: url(:/process/process/icon.png);border:none;color:rgb(38, 38, 38); }");
...
void windowLogin::_quit_clicked() {
    ...
    if (str1.contains("mainApp_new.exe"))                // ********************* TO BE DONE (NAME MAY BE CHANGED) ***************************
    {
        QProcess::startDetached("taskkill -t  -f /IM " + QString("mainApp_new.exe"));
