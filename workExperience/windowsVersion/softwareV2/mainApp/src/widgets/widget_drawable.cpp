...
#include "../include/easy3d_inc/widgets/widget_drawable.h"
...
void WidgetDrawable::setScalarFieldClamp(bool b) {
    ...
    if(!b)
    window_->log->set_log(log_type::Info, QString(tr("clamp range: both the lower and upper have returned to 0.")).toStdString(), window_->getLogWidget());
}
