...
std::string widget_newProcess::getWindowTimer() {
    if (window_)
...
void widget_newProcess::startWindowTimerP() {
    if (window_)
        window_->startTimerP();
}
std::vector<std::string> widget_newProcess::getWindowTimerP() {
    if (window_)
        return window_->getTimerP();

    std::vector<std::string> a;
    return a; // just for 'warning'
}
void widget_newProcess::stopWindowTimerP() {
    if (window_)
        window_->stopTimerP();
}
...
void widget_newProcess::windowAddToCmd(QString s) {
    window_->addToCmd(s);
}
...
void widget_newProcess::setSlamParam(
    ...
    this->loopInterior = loopInterior;
    ...
    list_slamParam.append(QVariant(this->loopInterior).toString());
...
void widget_newProcess::setRtkParam(
    ...
    //std::cout << this->outputCoordinateSystem.toStdString() << std::endl;
}
// ======================================================================================================================================
// ===============================================     Colourisation     ================================================================
...
void widget_newProcess::colorCrash() {

    if (colorErr == "merge") {
        setLog(tr("Merge mp4 files failed."));
        windowAddToCmd(tr("Merge mp4 files failed."));
        merge_mp4.close();
    }
    else if (colorErr == "colorization") {
        setLog(tr("Colorization failed."));
        windowAddToCmd(tr("Colorization failed."));
        _colour.close();
    }
    QProcess process;
    ...
    // set 
    labelRunning->setText(tr("canceling.."));
    QApplication::processEvents(); // show loading dialog
    QEventLoop loop;
    QTimer::singleShot(2000, &loop, SLOT(quit()));
    loop.exec();
    stopWindowTimerP();
    labelRunning->setText(tr("failed"));
    setLog(tr("Failed."));
    setStatus(running_status::finished);
    windowCanRunBag();
...
void widget_newProcess::manualGcpShow(QString a) {
    
    window_->open(a.toStdString());
    window_->prepareManualGcp("start");
    connect(window_, SIGNAL(_toAddxyzGcp(double,double,double)), this, SLOT(_dialogGcpxyz(double,double,double)), Qt::UniqueConnection);
    // start inputs
    if (!dialogGcp) {


        dialogGcp = new dialog_gcp(this);
        connect(dialogGcp, SIGNAL(_continue()), this, SLOT(_dialogGcpContinue()), Qt::UniqueConnection);
        connect(dialogGcp, SIGNAL(_closed()), this, SLOT(_dialogGcpClose()), Qt::UniqueConnection);

        //dialogGcp->gcpTxt = "E:\\vsTest\\11.txt";
        //dialogGcp->savePath = "E:\\vsTest";
        //dialogGcp->fileName = "aaa";
        QString path = getSlamParam().at(15) + "/";
        dialogGcp->savePath = path.replace('/', '\\');
        QString name = getSlamParam().at(14);
        dialogGcp->fileName = name.replace('/', '\\');
        dialogGcp->gcpTxt = gcpTxtFile.replace('/', '\\');
...
void widget_newProcess::manualGcpEnd() {
    window_->prepareManualGcp("end");
    disconnect(window_, SIGNAL(_toAddxyzGcp(double, double, double)), this, SLOT(_dialogGcpxyz(double, double, double)));
    if (dialogGcp) {
        dialogGcp->close();
        dialogGcp = nullptr;
        std::cout << "gcp - manual dialog closed. " << std::endl;
    }
    if (labelRunning->text() == "waiting for selection..") {
        process.shutAll(1);
    }
}
void widget_newProcess::_dialogGcpContinue() {
    labelRunning->setText("running..");
    window_->tabWidget->setCurrentWidget(window_->tabProcess);
    manualGcpEnd();
    process._gcpEEManual();
    
}
void widget_newProcess::_dialogGcpClose() {
   manualGcpEnd();
}
void widget_newProcess::_dialogGcpxyz(double x, double y, double z) {
    std::cout << "x "<<x<<" y "<<y<<" z "<<z << std::endl;
    // add to dialogGcp
    if (dialogGcp) {
        dialogGcp->addxyz(x,y,z);
...
QString widget_newProcess::addQuote(std::string str) {

    std::stringstream ss;
    //std::cout << "before: " << app << std::endl;
    ss << std::quoted(str);
    //QString app1 = QString::fromStdString(ss.str());
    //std::cout << "after: " << app1.toStdString() << std::endl;
