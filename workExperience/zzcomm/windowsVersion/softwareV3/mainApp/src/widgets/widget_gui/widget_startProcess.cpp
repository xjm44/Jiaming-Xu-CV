...
widget_startProcess::widget_startProcess(mainApp* parent)
	: QDialog(parent),
	slamBag(QString()),colorFileMp4(QString()), colorFilePc(QString()),rtkFile(QString()),gcpFile(QString()), productParam(QString())
...

{
	...
	//this->setMinimumHeight(size.height()*0.7);
...
void widget_startProcess::init() {
	...
	connect(btnMask, SIGNAL(clicked()), this, SLOT(_colorMask()), Qt::UniqueConnection);
	...
	frameRtk->hide();
	frameGcp->hide();
	//tabSetting->setTabEnabled(1, false);
	//tabSetting->setTabEnabled(2, false);
	//tabSetting->setTabEnabled(3, false);

	tabSetting->removeTab(3);
	tabSetting->removeTab(2);
	tabSetting->removeTab(1);
	//tabSetting->removeTab(0); // save slam first
	// ====================
	...
	checkBoxInterior->setDisabled(true);
	...
	// colourisation =========================
	QString cali = "..\\Lib\\company\\camera-calibGp.yaml";   // each camera has its own cali-file *********
	QFileInfo fi(cali);
	QString caliPath = fi.absoluteFilePath().replace('/', '\\');
	if(lineEditColorMp4->text().isEmpty())
		lineEditCali->setText(caliPath);
	...
	QFileInfo fi1(cP);
	QString cPPath = fi1.absoluteFilePath().replace('/', '\\');
...
void widget_startProcess::setParam(int i, widget_newProcess* np) {
	if (i == 0) { // slam
		...
		double frameskip = spinBoxFrameSkip->value();
		double firstframe = spinBoxFrameStart->value();
		double lastframe = spinBoxFrameEnd->value();
		...
		QString mask = lineEditMask->text();

		np->config = lineEditConfig->text(); // to be deleted. used for manual inputs now

		np->setColourParam(proj, imu, traj, cali, frameskip, firstframe, lastframe, visiblerange, quality, unclorPts, penetration,mask);
		...
		bool manualCheck = checkBoxManual->isChecked();
...
void widget_startProcess::_start() {	
	...
	if (check == QMessageBox::Yes) {
		...
		if (comboBoxType->currentIndex() == 0) {
			if (slamBag.isEmpty()) {
				QMessageBox::warning(nullptr,QString(tr("Warning")), QString(tr("Invalid .bag file.")));
				return;
			}
		}
		else if (comboBoxType->currentIndex() == 1) {
			if (colorFileMp4.isEmpty() || colorFilePc.isEmpty()) {
				QMessageBox::warning(nullptr, QString(tr("Warning")), QString(tr("Invalid file inputs.")));
				return;
			}
			else if (lineEditImu->text().isEmpty() || lineEditTraj->text().isEmpty()) {
				QMessageBox::warning(nullptr, QString(tr("Warning")), QString(tr("Missing file inputs.")));
				return;
			}
			else if (colorFileMp4.size()==1 && colorFileMp4.at(0).contains(".insv")) {
				if (lineEditMask->text().isEmpty()) {
					QMessageBox::warning(nullptr, QString(tr("Warning")), QString(tr("Missing mask file.")));
					return;
				}
				else if (!lineEditCali->text().contains("Pa.yaml")) {
					QMessageBox::warning(nullptr, QString(tr("Warning")), QString(tr("Invalid calibration file.")));
					return;
				}
			}
			else if (colorFileMp4.at(0).contains(".MP4") || colorFileMp4.at(0).contains(".mp4")) {
				if (!lineEditCali->text().contains("Gp.yaml")) {
					QMessageBox::warning(nullptr, QString(tr("Warning")), QString(tr("Invalid calibration file.")));
					return;
				}else if (!lineEditMask->text().isEmpty()) {
					QMessageBox::warning(nullptr, QString(tr("Warning")), QString(tr("Please do not choose mask file for mp4 inputs.")));
					return;
				}
			}
		}
			...
			if (lineEditRtk->text().isEmpty()) {
				QMessageBox::warning(nullptr, QString(tr("Warning")), QString(tr("Missing gps input.")));
				return;
			}
...
void widget_startProcess::comboboxType(int i) {
	...
	case 0: //slam
		...
		//tabSetting->setFixedHeight(200);
		//tabSetting->setTabEnabled(0, true);
		//tabSetting->setTabEnabled(1, false);
		//tabSetting->setTabEnabled(2, false);
		//tabSetting->setTabEnabled(3, false);
		//
		//frameSlamSetting->show();
		//frameColorSetting->hide();
		//frameRtkSetting->hide();
		//frameGcpSetting->hide();
	...
	case 1: // color
		...
		label_27->setMinimumWidth(labelColor->width());
		//tabSetting->setTabEnabled(0, false);
		//tabSetting->setTabEnabled(1, true);
		//tabSetting->setTabEnabled(2, false);
		//tabSetting->setTabEnabled(3, false);

		//frameColorSetting->show();
		//frameSlamSetting->hide();
		//frameRtkSetting->hide();
		//frameGcpSetting->hide();
		break;
	case 2: // rtk
		...
		//tabSetting->setTabEnabled(0, false);
		//tabSetting->setTabEnabled(1, false);
		//tabSetting->setTabEnabled(2, true);
		//tabSetting->setTabEnabled(3, false);

		//frameColorSetting->hide();
		//frameSlamSetting->hide();
		//frameRtkSetting->show();
		//frameGcpSetting->hide();
		break;
	case 3: //gcp
		// first layout
		...
		//tabSetting->setTabEnabled(1, false);
		//tabSetting->setTabEnabled(2, false);
		//tabSetting->setTabEnabled(3, true);

		//frameColorSetting->hide();
		//frameSlamSetting->show();
		//frameRtkSetting->hide();
		//frameGcpSetting->show();
		break;
	default:
		break;
	}
...
void widget_startProcess::comboboxEnvType(int i) {
	...
	case 1: //indoor
		...
		checkBoxInterior->setChecked(true);
		break;
	case 2: // outdoor
	...
	case 5: //uav,drone
		...
		doubleSpinBox_lidarMaxRange->setValue(60.0);
...
void widget_startProcess::_checkBox_loopClosure(int b) {
	if (b) {
		...
		checkBoxInterior->setEnabled(true);
	}
	else {
		spinBox_intervalFrames->setDisabled(true);
		checkBoxInterior->setDisabled(true);
...
void widget_startProcess::setSlamParam(widget_newProcess* np) {
	...
	bool loopInterior = checkBoxInterior->isChecked();
...
void widget_startProcess::_colorMp4() {
	...
	if (files.size() > 1) {
		...
		if (insvCheck.contains(".insv")) {
			QMessageBox::StandardButtons buts = (QMessageBox::Ok);
			QString title = QMessageBox::tr("Warning");
			QString text = QMessageBox::tr("Invalid video files chosen.\nPlease choose n MP4 files or 1 insv file only.");
			QMessageBox msgB(nullptr);
			msgB.setIcon(QMessageBox::Icon::Critical);
	...
	colorFileMp4Path = colorFileMp4Path.replace('/', '\\');
...
void widget_startProcess::autoFillColor() {
	...
	for (QString s : csvs) { // filter out csv
		//std::cout << s.toStdString() << std::endl;
		QRegularExpression regex("_begin|_optimized");
		QRegularExpressionMatchIterator i = regex.globalMatch(s);
		while (i.hasNext()) {
			QRegularExpressionMatch match = i.next();
			if (match.hasMatch()) {
...
void widget_startProcess::_colorMask() {
	QString file;
	//if (colorFileMp4Path != NULL) {
	//	file = QFileDialog::getOpenFileName(
	//		this,
	...
	file = QFileDialog::getOpenFileName(
		...
		"Png File",
		"..\\Lib\\company\\",
		"Supported formats (*.png)" // TO BE CHANGED
	);
	...
	QFileInfo fi(file);
	QString caliPath = fi.absolutePath().replace('/', '\\');
	caliPath += "\\camera-calibPa.yaml";
	lineEditCali->setText(caliPath);
}
...
void widget_startProcess::resetAllParams() {
	...
	checkBoxInterior->setChecked(false);
	...
	doubleSpinBoxalgorithm2Uphold->setValue(0.0);
