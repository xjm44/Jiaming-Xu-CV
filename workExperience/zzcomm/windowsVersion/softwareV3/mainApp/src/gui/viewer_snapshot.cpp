...
bool PaintCanvas::saveSnapshot(int w, int h, const QString& file_name) {
    int max_samples = 0;
    makeCurrent();
    ...
    if (image.isNull()) {
        QMessageBox::warning(0, "Image saving error", "Failed to allocate the image", QMessageBox::Ok,
...
void PaintCanvas::recordAnimation(const QString& file_name, int, int, bool bk_white) {
    ...
    for (std::size_t frame_index = 0; frame_index < frames.size(); ++frame_index) {
        ...
        if (image.isNull()) {
            QMessageBox::critical(0, "Error", "Failed to grab the framebuffer!");
            success = false;
            break;
        }
#else
        ...
        if (!image.save(full_name)) {
            QMessageBox::critical(0, "Error", QString("failed to save the %1-th frame").arg(frame_index));
            success = false;
            break;
        }
