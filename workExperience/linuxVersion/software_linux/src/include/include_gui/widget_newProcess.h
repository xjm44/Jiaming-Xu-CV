...
public:
    ...
    std::vector<std::string> getWindowTimer(); // seconds only, for each stage/function
    void stopWindowTimer(); // when cancel btn pressed
    ...
    // no_of_cores,las,csv,rotatingFrames,loopClosure,loopClosureIntervalFrames,narrowSpace,minIntensityFilter,lidarMaxRange,lidarMinRange,lidarMaxXToFilter,lidarMinXToFilter,translationThreshold,icpMaximumIterations,bagName,bagPath,algorithm2,sigma,laz,resolution,cuda,voxel,productParams
    ...
    // projDir,imu,traj,cali,frameskip,firstframe,lastframe,visiblerange
    void setColourParam(
        QString proj,
        QString imu,
        QString traj,
